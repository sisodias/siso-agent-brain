#!/usr/bin/env python3
"""SISO Agent Brain — a small shared-state HTTP service.

One authoritative SQLite database is written through this API. Agents on any
machine use the client contract instead of opening the database directly.

Design:
  - stdlib only (http.server + sqlite3). No external deps.
  - One service process is the database writer. Clients call the API.
  - Auth = split tokens: READ < WRITE < SPAWN. A leaked read token cannot write/spawn.
  - Transport binds to loopback by default; remote exposure is an operator concern.
  - Migrations create the complete core schema in an empty database.

Run:  python3 src/server.py
Env:  SISO_DB    (default ~/.local/share/siso-agent-brain/brain.db)
      SISO_BRAIN_PORT (default 8830)
      SISO_BRAIN_BIND (default 127.0.0.1)
      SISO_BRAIN_TOKENS_DIR (default ~/.config/siso/brain-tokens)
"""
import json, os, sqlite3, sys, time, uuid, hmac, urllib.parse
from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer

HOME = os.path.expanduser("~")
DB_PATH = os.environ.get("SISO_DB", f"{HOME}/.local/share/siso-agent-brain/brain.db")
PORT = int(os.environ.get("SISO_BRAIN_PORT", "8830"))
BIND = os.environ.get("SISO_BRAIN_BIND", "127.0.0.1")
TOKENS_DIR = os.environ.get("SISO_BRAIN_TOKENS_DIR", f"{HOME}/.config/siso/brain-tokens")
FACE_ENABLED = os.environ.get("SISO_BRAIN_FACE_ENABLED", "0") == "1"
...
<p class=mut style='margin-top:2em;font-size:.8em'>Served by SISO Agent Brain · use the configured Agent Zero or client to coordinate.</p>
</body></html>"""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/" or path == "/face":
            if not FACE_ENABLED:
                return self._send(404, {"error": "face disabled", "hint": "enable only on a trusted interface"})
            # The Face: a self-contained dashboard rendering the live brain.
            # The shell renders a server-side snapshot. Keep the service bound to
            # a trusted interface or place it behind an authenticated proxy.
            try:
                c = _db()
        ...
        if path == "/health":
            ...
            try:
                ...
                c.close()
                return self._send(200, {"ok": True, "database": "reachable", "timeline_events": n,
        ...
        if path == "/gate":
            ...
            #   capability->T1 (evolve freely; auto-promote after N clean uses) | disposable->T0 (auto-vault)
            if not self._auth("read"):
                return
            ...
            return self._send(200, {"decision": decision, "tier": tier, "unit": unit, "unit_tier": unit_tier,
                                    "note": "contract=the LOOP (always gated); capability incl. the layout (evolve freely)"})
    ...
    def do_POST(self):
        ...
        if path == "/ask":
            ...
            # Asking also raises a notification so the human sees it.
...
def main():
    ...
    sys.stderr.write(f"siso-agent-brain listening on {BIND}:{PORT} | db={DB_PATH} | tiers={sorted(set(TOKENS.values()))}\n")
    srv.serve_forever()
