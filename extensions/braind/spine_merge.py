...
Design:
  - Per-table INSERT OR IGNORE by PK for all SHARED tables (tables present in both DBs).
  ...
  - Tables completely excluded (machine-local state, not safe to union-merge):
      forge_drain_breaker — machine-local breaker circuit state (laptop-only table)
      schema_version      — machine-local migration bookkeeping (Mini-only table)
      mini_services       — Mini-specific service registry (Mini-only table)
      control_flags       — Mini-specific runtime flags (Mini-only table)
      challenges          — Mini-only (purpose unclear, machine-coupled)
      cost_events         — machine-local cost ledger
      farm_candidates     — machine-local farm queue
      division_inbox      — machine-local inbox queue
      divisions           — machine-local org structure
      notifications       — machine-local notification queue
      pending_questions   — machine-local pending Q queue
      scheduled_triggers  — machine-local scheduler state (live jobs)
      units               — Mini-only tier-management table
      idea_outcome        — laptop-only, empty, no Mini counterpart
  - session_harvest PK is `path` (not `id`) — handled explicitly.
...
# Tables to NEVER merge regardless of which side they're on.
# Document the reason for each exclusion above.
EXCLUDED_TABLES = frozenset([
    "forge_drain_breaker",   # machine-local breaker state
    "schema_version",         # machine-local migration bookkeeping
    "mini_services",          # Mini-specific service registry
    "control_flags",          # Mini-specific runtime flags
    "challenges",             # Mini-only, purpose unclear
    "cost_events",            # machine-local cost ledger
    "farm_candidates",        # machine-local farm queue
    "division_inbox",         # machine-local inbox queue
    "divisions",              # machine-local org structure
    "notifications",          # machine-local notification queue
    "pending_questions",      # machine-local pending Q
    "scheduled_triggers",     # machine-local scheduler state (live jobs)
    "units",                  # Mini-only tier-management
    "idea_outcome",           # laptop-only, 0 rows, no Mini counterpart
])

# Tables whose PK is NOT a single `id` column — map table → explicit PK column(s)
CUSTOM_PK = {
    "session_harvest": ["path"],
}
...
def merge(
    ...
    shared_tables = sorted((src_tables & tgt_tables) - EXCLUDED_TABLES)

    if verbose:
        laptop_only = sorted(src_tables - tgt_tables - EXCLUDED_TABLES)
        mini_only = sorted(tgt_tables - src_tables - EXCLUDED_TABLES)
        print(f"\nShared tables (will merge): {len(shared_tables)}")
        print(f"Source-only tables (skip): {laptop_only}")
        print(f"Target-only tables (skip): {mini_only}")
        print(f"Excluded tables (skip):    {sorted(EXCLUDED_TABLES & (src_tables | tgt_tables))}")
        print()
    ...
    for table in shared_tables:
        ...
        pk_cols = CUSTOM_PK.get(table, ["id"])
